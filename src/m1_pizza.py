...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(150.0, 50.0), rg.Point(120.0, 80.0), rg.Point(90.0, 50.0), rg.Point(120.0, 20.0)]
    circle = rg.Circle(rg.Point(120, 50), 30)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for _ in range (len(points)):
        circle = rg.Circle(points[_], 10)
        circle.fill_color = color
        circle.attach_to(window)
        points[_].attach_to(window)
    window.render()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for i in range(len(point)):
        line = rg.Line(circle.center, point[i])
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON tests 3:  4 segments black lines'
    window = rg.RoseWindow(550, 400, title)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for i in range(len(point)):
        line = rg.Line(point[i],point[i-1])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
    ...
    """
    # ------------------------------------------------------------------
    # TODDO: 8. Implement and test this function.
    #   Note that you should write its TEST function first (above).
    #
    # IMPLEMENTATION REQUIREMENT:
    #    You MUST USE (call) the   generate_points_on_circle
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4: 300 hop 61 yellow.'
    window = rg.RoseWindow(480, 350, title)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for i in range(len(point)):
        line = rg.Line(point[i], point[i - hops_to_next_point])
        line.thickness = thickness
        line.color = color
        line.arrow = "first"
        line.attach_to(window)
